...
from utils.layers import conv3d_transpose, dis_block, conv3d, linear
from utils.utils import saveGIFBatch, sampleBatch


class ImprovedVideoGANInpaint(object):
    def __init__(self,
                 input_batch,
                 box_size=20,
                 batch_size=64,
    ...
    def mask_video(self, batch):
        # generate random location for left-top corner of box on image
        x = 23  # random.randint(0, self.crop_size-self.box_size)
        y = 23  # random.randint(0, self.crop_size-self.box_size)

        mask = np.ones([self.batch_size, self.frame_size, self.crop_size, self.crop_size, 3], np.float32)
        mask[:, :, x:x + self.box_size, y:y + self.box_size, :] = 0.
        mask_tensor = tf.constant(mask, tf.float32, name='video_mask')
        batch = np.multiply(batch, mask_tensor)
        return batch

    def generator(self, batch):
        self.unmasked_video = batch
        self.masked_video = self.mask_video(batch)

        with tf.variable_scope('g_') as vs:
            """ -----------------------------------------------------------------------------------
            ENCODER 
            ----------------------------------------------------------------------------------- """
            self.en_h0 = conv3d(self.masked_video, 3, 64, name="enc_conv1")
    ...
    def build_model(self):
        ...
        self.sample_masked = sampleBatch(self.masked_video, self.batch_size)
        self.sample_unmasked = sampleBatch(self.unmasked_video, self.batch_size)
    ...
    def train(self,
        ...
        if generate_sample:
            feed_dict = self.get_feed_dict(session)
            vid_sample, masked_sample, groundT = session.run([self.sample, self.sample_masked, self.sample_unmasked],
                                                             feed_dict=feed_dict)
            saveGIFBatch(vid_sample, sample_dir, 'vid_%d_out' % step)
            saveGIFBatch(groundT, sample_dir, 'vid_%d_gt' % step)
            saveGIFBatch(masked_sample, sample_dir, 'vid_%d_in' % step)
